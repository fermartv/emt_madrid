...
from homeassistant.const import ATTR_LATITUDE, ATTR_LONGITUDE, HTTP_OK
...
class AemetApi:
    ...
    def update(self):
        ...
        endpoint_url = "{}{}".format(
            self.API_URL_BASE, self.API_STATION_ENDPOINT.format(self._station_id)
        )
        params = {"api_key": self._api_key}
        main_rsp = requests.get(endpoint_url, params=params, verify=True)
        ...
        if main_result["estado"] == HTTP_OK:
            ...
            data_rsp = requests.get(hashed_endpoint, verify=True)
